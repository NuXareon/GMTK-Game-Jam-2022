...
public class PerryScript : MonoBehaviour
{
    ...
    public HarmonyProjectBinary PaletteDefault;
    public HarmonyProjectBinary PaletteAutumn;
    private bool hasPaletteChanged = false;
    ...
    {
        //Changing palette means using different HarmonyProjectBinary as long as the only change in them are the drawings used
        //This will not work if it's with two complete characters with their own unique drawings and naming convention
        //HarmonyProjectBinary with similar naming convention (ie. exported from the same Harmony Scene but with a different palette) can share Animators
        if(paletteChanged)
        {
          switch(Palette)
          {
            case 0:
              Renderer.Project = PaletteDefault;
              hasPaletteChanged = true;
              break;

            case 1:
              Renderer.Project = PaletteAutumn;
              hasPaletteChanged = true;
              break;
          }
        }
        ...
        if(eyesChanged)
        {
          switch(Eyes)
          {
            case 0:
              ...
              break;
            case 1:
              Renderer.GroupSkins.SetSkin(0,5);
              break;
          }
        }
        ...
        if(glassesChanged)
        {
          switch(Glasses)
          {
            ...
            case 1:
              ...
              break;
            case 2:
              Renderer.GroupSkins.SetSkin(1,3);
              break;
          }
        }
        ...
        if(headbandChanged)
        {
          ...
          {
            case 0:
              Renderer.GroupSkins.SetSkin(2,1);
              break;
            case 1:
              Renderer.GroupSkins.SetSkin(2,6);
              break;
          }
        }
